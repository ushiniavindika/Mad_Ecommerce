...
class SingleProductViewController: BaseViewController {
    ...
    @IBAction func plusCartAction(_ sender: Any) {
        ...
        if currentCountInt! < 10{
            ...
            showAlert(title: "We are sorry", message: "Maximum number of items is limited to 10")
        }
    }
    
    func configureSinglePage(with product:Product){
        ...
        if let price = product.prodPrice{
            ...
            priceWithTaxLabel.text = "Rs\(String(format: "%.2f", priceWithTax)) incl. GST"
        }
